...
const TestimonialsImages: React.FC<TestimonialsImagesProps> = ({
	...
	const carouselMedium = useMediaQuery("(min-width: 1024px)");
	...
	return (
		<div className="px-4 md:px-8 lg:px-24 h-[10vh] md:h-full mt-4">
			<AliceCarousel
				mouseTracking
				responsive={responsive}
				controlsStrategy="alternate"
				disableDotsControls={true}
				disableButtonsControls={carouselMedium}
			>
					...
					return (
						<div
							...
							className={`rounded-full border-2 cursor-pointer ${
								matchesMedium
									? index === 2
										? "opacity-100 w-36 h-36 border-primary-blueText transition-transform transform hover:scale-110 hover:border-opacity-0"
										: "opacity-20 w-20 h-20 transition-transform transform hover:scale-110"
									: "opacity-100 w-24 h-24 border-primary-blueText transition-transform transform hover:scale-110 hover:border-opacity-0"
							}`}
