...
const OngoingProjects: React.FC = () => {
  ...
  return (
    <div className="my-12 text-primary-blueText px-4 md:px-12 lg:px-16">
      ...
      <AliceCarousel infinite mouseTracking disableDotsControls 
       renderPrevButton={() => {
        return (
          <button
      ...
      >
        {myOngoingprojects.map((item) => {
            return(
                <div key={item.id} className="flex items-center justify-between flex-col md:flex-row">
                    <div className="w-[82%] md:w-[40%] h-[50vh] mb-12 md:mb-0">
                    <img src={item.image} alt="ongoing-project" className="w-full h-full"/>
                    </div>
               
                <div className="bg-secondary-detailsBackground py-12 px-4 md:px-8 text-black w-[82%] md:w-[50%]">
                  <h1 className="text-2xl md:text-3xl font-playfair pb-4 ">{item.title}</h1>
