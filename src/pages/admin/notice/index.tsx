...
const AdminNotice = () => {
	const [data, loading, error] = useCollectionData(collection(db, "notice"));
	...
	return (
		<>
			<div>
				<h1 className="text-3xl font-bold mb-8 pl-2">Notices</h1>
					...
					<TableBody>
						{data?.map((notice) => (
							<TableRow className="border-2" key={nanoid()}>
								<TableCell className="border-2 p-2">{notice.title}</TableCell>
								<TableCell
									className="border-2"
									dangerouslySetInnerHTML={{
										__html: truncateOverview(notice.description, 185),
									}}
								/>
